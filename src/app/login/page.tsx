...
export default function LoginPage(){
        ...
        const login = async () => {
           try {
            setLoading(true)
             ...
             console.log("Login Failed 1",error.message)
           } finally {
            setLoading(false);
           }
        }
    ...
    <div className="w-full max-w-md bg-white p-6 rounded-lg shadow-md">
        <h1 className="text-center text-3xl font-semibold text-gray-700">{loading?"processing":"Login"}</h1>
        <br />
        <div className="space-y-4">
            ...
            <button onClick={login} className="w-full bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600 transition duration-300">
                Login 
            </button>
            <p className="text-center text-gray-600 mt-4">
                <span> Don't have an account? </span>
                <Link href="/signup" className="text-blue-500 hover:underline ml-1">SignUp</Link>
            </p>
        </div>
